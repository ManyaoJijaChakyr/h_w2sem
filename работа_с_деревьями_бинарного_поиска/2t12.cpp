...
void Delete(tree*& tr, tree* v) {//удаление узла
	...
	if (!p) tr = NULL; //дерево содержит один узел
		...
		if (!p) { //если удаляем корень, у которого 1 ребенок
			...
			if (!v->left) {//если есть правый ребенок
				if (p->left == v) //если удаляемый узел явл. левым ребенком
					p->left = v->right; //ребенок удаляемого узла становится левым ребенкомсвоего "деда"
				else
					p->right = v->right; ////ребенок удаляемого узла становится правымребенком своего "деда"
		...
		if (succ->parent->left == succ) {//если succ левый ребенок
			succ->parent->left = succ->right; //его правый ребенок становится левымребенком своего "деда"
...
void deleteMultiples(tree*& tr, int X) {
	if (tr == NULL) return; //если дерево пустое, выходим из функции
	if (tr->inf % X == 0) { //если корень дерева кратен X, удаляем его
		Delete(tr, tr);
		deleteMultiples(tr, X); //рекурсивно вызываем функцию для нового корня
	}
	else { //если корень дерева не кратен X
		deleteMultiples(tr->left, X); //рекурсивно вызываем функцию для левого поддерева
		deleteMultiples(tr->right, X); //рекурсивно вызываем функцию для правого поддерева
	}
}
