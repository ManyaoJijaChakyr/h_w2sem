...
int height(tree* tr) {
	if (tr == NULL) return 0; //если дерево пустое, высота равна 0
	int leftHeight = height(tr->left); //вызываем функцию для левого поддерева
	int rightHeight = height(tr->right); //вызываем функцию для правого поддерева
	return max(leftHeight, rightHeight) + 1; //возвращаем максимальную высоту поддерева плюс 1 (для текущего узла)
}
